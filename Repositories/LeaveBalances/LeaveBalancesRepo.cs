...
namespace AttendanceSystemBackend.Repositories.LeaveBalances
{
    public class LeaveBalancesRepo : ILeaveBalancesRepo
    {
        ...
        public async Task AddAsync(Models.LeaveBalance balance)
        {
            using var connection = CreateConnection();
        ...
        public async Task AdjustLeaveBalanceAsync(string employeeId, string leaveTypeId, decimal days)
        {
            using var connection = CreateConnection();
            var sql = @"UPDATE LeaveBalances SET remainingDays = remainingDays - @Days
